...
#define CHECK_ASSERTIONS(assertions)                                                               \
    (((assertions & ASSERT_AT_BOL) && (pos > 0 || reg_notbol) &&                                   \
     ...
     ((assertions & ASSERT_AT_EOL) && (next_c != L'\0' || reg_noteol) &&                           \
      (next_c != L'\n' || !reg_newline)) ||                                                        \
     ((assertions & ASSERT_AT_BOW) && (IS_WORD_CHAR(prev_c) || !IS_WORD_CHAR(next_c))) ||          \
     ((assertions & ASSERT_AT_EOW) && (!IS_WORD_CHAR(prev_c) || IS_WORD_CHAR(next_c))) ||          \
     ((assertions & ASSERT_AT_WB) &&                                                               \
      (pos != 0 && next_c != L'\0' && IS_WORD_CHAR(prev_c) == IS_WORD_CHAR(next_c))) ||            \
...
inline static int tre_tag_order(int num_tags, tre_tag_direction_t *tag_directions, int *t1, int *t2)
{
    int i;
