...
static unsigned int hash_void_ptr(void *ptr)
{
    unsigned int hash;
    unsigned int i;
...
static void hash_table_add(hashTable *tbl, void *ptr, size_t bytes, const char *file, int line,
                           const char *func)
{
    unsigned int i;
...
int xmalloc_dump_leaks(void)
{
    ...
    printf("Peak memory consumption %d bytes (%.1f kB, %.1f MB) in %d blocks ", xmalloc_peak,
           (double)xmalloc_peak / 1024, (double)xmalloc_peak / (1024 * 1024), xmalloc_peak_blocks);
    printf("(average ");
    if (xmalloc_peak_blocks)
    {
        printf("%d", ((xmalloc_peak + xmalloc_peak_blocks / 2) / xmalloc_peak_blocks));
    }
    else
    {
        printf("N/A");
...
void *xcalloc_impl(size_t nmemb, size_t size, const char *file, int line, const char *func)
{
    void *ptr;
...
void *xrealloc_impl(void *ptr, size_t new_size, const char *file, int line, const char *func)
{
    void *new_ptr;
    size_t old_size;
