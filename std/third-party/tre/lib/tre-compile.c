...
static reg_errcode_t tre_copy_ast(tre_mem_t mem, tre_stack_t *stack, tre_ast_node_t *ast, int flags,
                                  int *pos_add, tre_tag_direction_t *tag_directions,
        ...
        case EXPAND_AFTER_ITER:
        {
            ...
            if (iter->min > 1 || iter->max > 1)
            {
                ...
                for (j = 0; j < iter->min; j++)
                {
                    ...
                    status = tre_copy_ast(mem, stack, iter->arg, flags, &pos_add, tag_directions,
                                          &copy, &max_pos);
                    if (status != REG_OK)
                    {
                        return status;
            ...
            if (iter->params)
            {
                ...
                old_params = tre_mem_alloc(mem, sizeof(*old_params) * TRE_PARAM_LAST);
                if (!old_params)
                {
                    return REG_ESPACE;
                }
                ...
                node_copy = tre_ast_new_node(mem, ITERATION, sizeof(tre_iteration_t));
                if (!node_copy)
                {
                    return REG_ESPACE;
                }
...
static tre_pos_and_tags_t *tre_set_union(tre_mem_t mem, tre_pos_and_tags_t *set1,
                                         ...
                                         int *params)
{
    ...
    for (s1 = 0; set1[s1].position >= 0; s1++)
    {
        ...
        if (params)
        {
            ...
            {
                new_set[s1].params = tre_mem_alloc(mem, sizeof(*params) * TRE_PARAM_LAST);
                if (!new_set[s1].params)
                {
                    return NULL;
                }
    ...
    for (s2 = 0; set2[s2].position >= 0; s2++)
    {
        ...
        if (params)
        {
            ...
            {
                new_set[s1 + s2].params = tre_mem_alloc(mem, sizeof(*params) * TRE_PARAM_LAST);
                if (!new_set[s1 + s2].params)
                {
                    return NULL;
                }
...
static reg_errcode_t tre_compute_npfl(tre_mem_t mem, tre_stack_t *stack, tre_ast_node_t *tree,
                                      int *nextpos)
{
    size_t bottom = tre_stack_num_items(stack);
    ...
    while (tre_stack_num_items(stack) > bottom)
    {
        ...
        case NPFL_RECURSE:
            ...
            case LITERAL:
            {
                ...
                else
                {
                    ...
                    lit->position = (*nextpos)++;
                    node->firstpos =
                        tre_set_one(mem, lit->position, lit->code_min, lit->code_max, 0, NULL, -1);
                    if (!node->firstpos)
                    {
                        return REG_ESPACE;
                    }
        ...
        case NPFL_POST_UNION:
        {
            ...
            node->firstpos =
                tre_set_union(mem, uni->left->firstpos, uni->right->firstpos, NULL, 0, NULL);
            if (!node->firstpos)
            {
                return REG_ESPACE;
            }
            node->lastpos =
                tre_set_union(mem, uni->left->lastpos, uni->right->lastpos, NULL, 0, NULL);
            if (!node->lastpos)
            {
                return REG_ESPACE;
            }
        ...
        case NPFL_POST_CATENATION:
        {
            ...
            if (cat->left->nullable)
            {
                /* The left side matches the empty string.  Make a first pass
                   ...
                   parameters. */
                status =
                    tre_match_empty(stack, cat->left, NULL, NULL, NULL, &num_tags, &params_seen);
                if (status != REG_OK)
                {
                    return status;
                }
                ...
                if (params_seen)
                {
                    params = tre_mem_alloc(mem, sizeof(*params) * TRE_PARAM_LAST);
                    if (!params)
                    {
                        xfree(tags);
                        return REG_ESPACE;
                ...
                /* Second pass with tre_mach_empty() to get the list of
                   tags and parameters. */
                status = tre_match_empty(stack, cat->left, tags, &assertions, params, NULL, NULL);
                if (status != REG_OK)
                {
                    xfree(tags);
                    return status;
                }
                node->firstpos = tre_set_union(mem, cat->right->firstpos, cat->left->firstpos, tags,
                                               assertions, params);
                xfree(tags);
                if (!node->firstpos)
                {
            ...
            if (cat->right->nullable)
            {
                /* The right side matches the empty string.  Make a first pass
                   ...
                   parameters. */
                status =
                    tre_match_empty(stack, cat->right, NULL, NULL, NULL, &num_tags, &params_seen);
                if (status != REG_OK)
                {
                    return status;
                }
                ...
                /* Second pass with tre_mach_empty() to get the list of
                   tags and parameters. */
                status = tre_match_empty(stack, cat->right, tags, &assertions, params, NULL, NULL);
                if (status != REG_OK)
                {
                    xfree(tags);
                    return status;
                }
                node->lastpos = tre_set_union(mem, cat->left->lastpos, cat->right->lastpos, tags,
                                              assertions, params);
                xfree(tags);
                if (!node->lastpos)
                {
		...
		if (trans->state_id == p2->position)
		  {
		    ...
		    break;
                ...
                if (p1->params || p2->params)
                {
                    if (!trans->params)
                    {
                        trans->params = xmalloc(sizeof(*trans->params) * TRE_PARAM_LAST);
                    }
                    if (!trans->params)
                    {
                        return REG_ESPACE;
...
#ifdef TRE_DEBUG
                {
                    ...
                    DPRINT(("	 %2d -> %2d on %3d", p1->position, p2->position, p1->code_min));
                                     ...
                                     int *counts, int *offs)
{
    ...
    case CATENATION:
       ...
       to each position in cat->right->firstpos. */
        errcode =
            tre_make_trans(cat->left->lastpos, cat->right->firstpos, transitions, counts, offs);
        if (errcode != REG_OK)
        {
            return errcode;
        }
...
int tre_compile(regex_t *preg, const tre_char_t *regex, size_t n, int cflags)
{
    ...
    if (tnfa->have_backrefs || !(cflags & REG_NOSUB))
    {
        ...
        tnfa->minimal_tags = xcalloc((unsigned)tnfa->num_tags * 2 + 1, sizeof(tnfa->minimal_tags));
        if (tnfa->minimal_tags == NULL)
        {
            ERROR_EXIT(REG_ESPACE);
        }

        submatch_data = xcalloc((unsigned)parse_ctx.submatch_id, sizeof(*submatch_data));
        ...
        {
            DPRINT(("t%d is %s\n", i,
                    tag_directions[i] == TRE_TAG_MINIMIZE ? "minimized" : "maximized"));
        }
#endif /* TRE_DEBUG */
    }

    /* Expand iteration nodes. */
    errcode = tre_expand_ast(mem, stack, tree, tag_directions, &tnfa->params_depth);
    if (errcode != REG_OK)
    {
        ERROR_EXIT(errcode);
    }
