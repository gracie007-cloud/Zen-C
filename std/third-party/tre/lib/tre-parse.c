...
static void tre_expand_macro(const tre_char_t *regex, const tre_char_t *regex_end, tre_char_t *buf,
                             size_t buf_len)
{
    int i;
...
static reg_errcode_t tre_new_item(tre_mem_t mem, int min, int max, int *i, int *max_i,
...
static reg_errcode_t tre_expand_ctype(tre_mem_t mem, tre_ctype_t char_class, tre_ast_node_t ***items,
                                      int *i, int *max_i, int cflags)
{
    ...
    {
        c = (tre_cint_t)j;
        if (tre_isctype(c, char_class) || ((cflags & REG_ICASE) && (tre_isctype(tre_tolower(c), char_class) ||
                                                               tre_isctype(tre_toupper(c), char_class))))
        {
            if (min < 0)
            {
                min = c;
...
static int tre_compare_items(const void *a, const void *b)
{
...
struct
{
    char *name;
    int (*func)(tre_cint_t);
} tre_ctype_map[] = {{"alnum", &tre_isalnum_func},   {"alpha", &tre_isalpha_func},
#ifdef tre_isascii
...
#endif /* tre_isblank */
                     {"cntrl", &tre_iscntrl_func},   {"digit", &tre_isdigit_func},
                     {"graph", &tre_isgraph_func},   {"lower", &tre_islower_func},
                     {"print", &tre_isprint_func},   {"punct", &tre_ispunct_func},
                     {"space", &tre_isspace_func},   {"upper", &tre_isupper_func},
                     {"xdigit", &tre_isxdigit_func}, {NULL, NULL}};
...
static reg_errcode_t tre_parse_bracket_items(tre_parse_ctx_t *ctx, int negate,
                                             ...
                                             int *items_size)
{
    ...
    reg_errcode_t status;
    tre_ctype_t char_class = (tre_ctype_t)0;
    ...
    for (;;)
    {
        ...
        else if (re + 1 < ctx->re_end && *re == CHAR_LBRACKET && *(re + 1) == CHAR_PERIOD)
        {
            return REG_ECOLLATE;
        }
        else if (re + 1 < ctx->re_end && *re == CHAR_LBRACKET && *(re + 1) == CHAR_EQUAL)
        {
            return REG_ECOLLATE;
        }
        else if (re + 1 < ctx->re_end && *re == CHAR_LBRACKET && *(re + 1) == CHAR_COLON)
        {
            char tmp_str[64];
            ...
            if (endptr != ctx->re_end)
            {
                ...
                if (ctx->mb_cur_max == 1)
                {
                    status = tre_expand_ctype(ctx->mem, char_class, items, &i, &max_i, ctx->cflags);
                    if (status != REG_OK)
                    {
                        return status;
                    }
        ...
        if (char_class && negate)
        {
            ...
            {
                neg_classes[(*num_neg_classes)++] = char_class;
            }
        }
        else if (!skip)
        {
            ...
            ((tre_literal_t *)((*items)[i - 1])->obj)->u.char_class = char_class;
        }

        /* Add opposite-case counterpoints if REG_ICASE is present.
       This is broken if there are more than two "same" characters. */
        if (ctx->cflags & REG_ICASE && !char_class && !skip)
        {
...
static reg_errcode_t tre_parse_bracket(tre_parse_ctx_t *ctx, tre_ast_node_t **result)
{
    ...
    status =
        tre_parse_bracket_items(ctx, negate, neg_classes, &num_neg_classes, &items, &i, &max_i);
...
static int tre_parse_int(const tre_char_t **regex, const tre_char_t *regex_end)
{
    unsigned long num = 0;
    int overflow = 0;
...
static reg_errcode_t tre_parse_bound(tre_parse_ctx_t *ctx, tre_ast_node_t **result)
{
    int min, max, i;
    ...
    */
    ...
    {
        if (r + 1 >= ctx->re_end || *r != CHAR_BACKSLASH || *(r + 1) != CHAR_RBRACE)
        {
            return REG_BADBR;
        }
        r += 2;
    ...
    DPRINT(("tre_parse_bound: min %d, max %d, costs [%d,%d,%d, total %d], "
            ...
            min, max, cost_ins, cost_del, cost_subst, cost_max, limit_ins, limit_del, limit_subst,
            limit_err));
...
#ifdef REG_LITERAL
            if (!(ctx->cflags & REG_LITERAL) && ctx->cflags & REG_EXTENDED)
#endif /* REG_LITERAL */
        ...
        case PARSE_CATENATION:
            /* If the expression has not ended, parse another piece. */
            {
                ...
                if (!(ctx->cflags & REG_LITERAL))
                {
                    ...
                    if ((ctx->cflags & REG_EXTENDED && c == CHAR_RPAREN && depth > 0) ||
                        (!(ctx->cflags & REG_EXTENDED) &&
                         (c == CHAR_BACKSLASH && *(ctx->re + 1) == CHAR_RPAREN)))
                    {
        ...
        case PARSE_POSTFIX:
            ...
            case CHAR_STAR:
            {
                ...
                if (ctx->re + 1 < ctx->re_end)
                {
                    ...
                    else if (*(ctx->re + 1) == CHAR_STAR || *(ctx->re + 1) == CHAR_PLUS)
                    {
                        /* These are reserved for future extensions. */
                        return REG_BADRPT;
                    }
                ...
                DPRINT(("tre_parse: %s star: '%.*" STRF "'\n", minimal ? "  minimal" : "greedy",
                        REST(tmp_re)));
                ctx->re++;
                tmp_node = tre_ast_new_iter(ctx->mem, result, rep_min, rep_max, minimal);
                if (tmp_node == NULL)
                {
                    return REG_ESPACE;
                }
        ...
        case PARSE_ATOM:
            ...
            case CHAR_LPAREN: /* parenthesized subexpression */
               ...
               to Perls corresponding extensions. */
                if (ctx->cflags & REG_EXTENDED && *(ctx->re + 1) == CHAR_QUESTIONMARK)
                {
                    ...
                    while (/*CONSTCOND*/ (void)1, 1)
                    {
                        ...
                        else if (*ctx->re == CHAR_HASH)
                        {
                            ...
                            /* A comment can contain any character except a
                               right parenthesis */
                            while (*ctx->re != CHAR_RPAREN && ctx->re < ctx->re_end)
                            {
                                ctx->re++;
                            }
                ...
                {
                    depth++;
                    if (ctx->re + 2 < ctx->re_end && *(ctx->re + 1) == CHAR_QUESTIONMARK &&
                        *(ctx->re + 2) == CHAR_COLON)
                    {
                        DPRINT(("tre_parse: group begin: '%.*" STRF "', no submatch\n",
                                REST(ctx->re)));
            ...
            case CHAR_BACKSLASH:
               ...
               try again. */
                if (!(ctx->cflags & REG_EXTENDED) && ctx->re + 1 < ctx->re_end &&
                    (*(ctx->re + 1) == CHAR_LPAREN || *(ctx->re + 1) == CHAR_RPAREN))
                {
                    ctx->re++;
                    STACK_PUSHX(stack, int, PARSE_ATOM);
                    break;
                ...
                {
                    tre_char_t buf[64];
                    tre_expand_macro(ctx->re + 1, ctx->re_end, buf, elementsof(buf));
                    if (buf[0] != 0)
                    {
                ...
                {
                case L'b':
                    result = tre_ast_new_literal(ctx->mem, ASSERTION, ASSERT_AT_WB);
                    ctx->re++;
                    break;
                case L'B':
                    result = tre_ast_new_literal(ctx->mem, ASSERTION, ASSERT_AT_WB_NEG);
                    ctx->re++;
                    break;
                case L'<':
                    result = tre_ast_new_literal(ctx->mem, ASSERTION, ASSERT_AT_BOW);
                    ctx->re++;
                    break;
                case L'>':
                    result = tre_ast_new_literal(ctx->mem, ASSERTION, ASSERT_AT_EOW);
                    ctx->re++;
                    break;
                case L'x':
                    ctx->re++;
            ...
            case CHAR_CARET: /* beginning of line assertion */
               ...
               beginning of the RE and after \( is BREs. */
                if (ctx->cflags & REG_EXTENDED ||
                    (ctx->re - 2 >= ctx->re_start && *(ctx->re - 2) == CHAR_BACKSLASH &&
                     *(ctx->re - 1) == CHAR_LPAREN) ||
                    ctx->re == ctx->re_start)
                {
                    DPRINT(("tre_parse:	      BOL: '%.*" STRF "'\n", REST(ctx->re)));
                    result = tre_ast_new_literal(ctx->mem, ASSERTION, ASSERT_AT_BOL);
                    if (result == NULL)
                    {
                        return REG_ESPACE;
                    }
            ...
            case CHAR_DOLLAR: /* end of line assertion. */
               ...
               string and before \) is BREs. */
                if (ctx->cflags & REG_EXTENDED ||
                    (ctx->re + 2 < ctx->re_end && *(ctx->re + 1) == CHAR_BACKSLASH &&
                     *(ctx->re + 2) == CHAR_RPAREN) ||
                    ctx->re + 1 == ctx->re_end)
                {
                    DPRINT(("tre_parse:	      EOL: '%.*" STRF "'\n", REST(ctx->re)));
                    result = tre_ast_new_literal(ctx->mem, ASSERTION, ASSERT_AT_EOL);
                    if (result == NULL)
                    {
                        return REG_ESPACE;
                    }
            ...
            parse_literal:
                ...
                if (temporary_cflags && ctx->re + 1 < ctx->re_end && *ctx->re == CHAR_BACKSLASH &&
                    *(ctx->re + 1) == L'E')
                {
               ...
               (which matches an empty string).  */
                if (
                    ...
                    !(ctx->cflags & REG_LITERAL) &&
#endif /* REG_LITERAL */
                    (ctx->re >= ctx->re_end || *ctx->re == CHAR_STAR ||
                     (ctx->cflags & REG_EXTENDED &&
                      (*ctx->re == CHAR_PIPE || *ctx->re == CHAR_LBRACE || *ctx->re == CHAR_PLUS ||
                       *ctx->re == CHAR_QUESTIONMARK))
                     /* Test for "\)" in BRE mode. */
                     || (!(ctx->cflags & REG_EXTENDED) && ctx->re + 1 < ctx->re_end &&
                         *ctx->re == CHAR_BACKSLASH && *(ctx->re + 1) == CHAR_LBRACE)))
                {
                    DPRINT(("tre_parse:	    empty: '%.*" STRF "'\n", REST(ctx->re)));
                    result = tre_ast_new_literal(ctx->mem, EMPTY, -1);
                    if (!result)
